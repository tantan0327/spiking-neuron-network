...
def computePCAmap(odim=2, transpo=False):
    ...
    print("PCA explains " + str(pcanode1.explained_variance) + " of the variance with " + str(
        len(proj[0, :])) + " dimensions.")
    return proj, expvar
...
def embedSystem(embedding, explained_variance):
    ...
    pl.title('Two-dimensional embedding explains ' + str(
        round(explained_variance, 3) * 100) + '% of variance among observed system states')
...
def embedIndividuals(embedding, explained_variance):
    ...
    pl.title('Two-dimensional embedding explains ' + str(
        round(explained_variance, 3) * 100) + '% of variance among individual neurons')
    pl.xlabel('Principal component 1')
    pl.ylabel('Principal component 2')
    pl.tick_params(labelleft='off', labelbottom='off')
...
for i in range(10):
    ...
    v = -65.0 * np.ones(N, dtype=double);
    u = b * v  # Initial values of u,v
